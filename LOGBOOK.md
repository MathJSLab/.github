...
## 2025-03-28
- The `src/` and `types/` directories were created in the organization repository to hold common source files and type definitions for projects. The `src/components/` directory was populated with common files (types and factories) from `mathjslab-app` to define web components. These files were added to the `resource/mathjslab-app/download.config.json` file. The `eslint.config.cjs` file was modified for this reason: a set of rules for these files was added. A `tsconfig.json` file in the root directory was created just to run ESLint.
